    ...
    public function orderProcessHandle($order_id,$act){
    	...
    	switch ($act){
    		case 'pay': //付款
                ...
                $order_user_id = M('order')->where("order_id = $order_id")->getField("user_id");
                $sql = "UPDATE __PREFIX__users SET stock = stock + 10 WHERE user_id = {$order_user_id}";
                D('users')->execute($sql);
                
                $tpStock = M("stock")->find();
                $tpStock["presented_stock_count"] = $tpStock["presented_stock_count"] + 10; //赠送股票
