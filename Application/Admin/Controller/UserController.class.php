...
class UserController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        {
            $first_leader = M('users')->query("select first_leader,count(1) as count  from tp_users where first_leader in(".  implode(',', $user_id_arr).")  group by first_leader");
            $first_leader = convert_arr_key($first_leader,'first_leader');
            
            $second_leader = M('users')->query("select second_leader,count(1) as count  from tp_users where second_leader in(".  implode(',', $user_id_arr).")  group by second_leader");
            $second_leader = convert_arr_key($second_leader,'second_leader');            
            
            $third_leader = M('users')->query("select third_leader,count(1) as count  from tp_users where third_leader in(".  implode(',', $user_id_arr).")  group by third_leader");
    ...
    public function stock(){
    	...
    	$sql = "select orders.user_id ,u.nickname,orders.order_amount,u.stock
    			  from ( select user_id,sum(order_amount) order_amount from __PREFIX__order o where o.order_status != 0 group by user_id ) orders
    			 inner join __PREFIX__users u on orders.user_id = u.user_id
    	...
    	if(IS_POST){
    		...
    		if($stockCount > 0){
	    		foreach ($users as $user){
	    			...
	    			$tpStock["presented_stock_count"] = $tpStock["presented_stock_count"] + $stockCount;//赠送的股票
	    			...
	    			$user_stock = array(
	    					...
	    					"pay_points" => 0,
	    					"status" => "1",
	    					"stock_amount" => $stockCount
	    			);
	    			M('user_stock_log')->add($user_stock);
	    			 
	    			$sql = "UPDATE __PREFIX__users SET stock = stock + $stockCount WHERE user_id = ".$user["user_id"];
	    			D('users')->execute($sql);
	    			
	    			$user["stock"] = $user["stock"] + $stockCount;
	    		}
    		}
    	}
    ...
    public function stocks()
    {
        ...
        $stock_amount = M("Users")->sum("stock");

        $one = $system_amount['amount'] / 100000000;
        $ones = sprintf("%.8f", $one);

        $total = $one * $stock_amount;

        $rem = $system_amount['amount'] - $total;

        $this->assign("system_amount", $system_amount);
        $this->assign("one", $ones);
        $this->assign("rem", $rem);
    ...
    {
        $stock = M("Stock_log");
        $data['system_amount'] = I("system_amount");
        $data['stock_amount'] = I("stock_amount");
        $data['stock_one'] = I("one");
        $data['stock_total'] = I("total");
        $data['stock_rem'] = I("rem");
        $data['add_time'] = time();
        $result =$stock->add($data);
        if ($result) {
            $system_amount = M("System_amount");
            $value['amount'] = 0;
            $system_amount->where("amount > 0")->save($value);

            $user = M("Users");
            $users = $user->select();
            foreach ($users as $k => $v) {
                $ke['user_money']= $users[$k]['stock'] * $data['stock_one']+ $users[$k]['user_money'];
                $user->where("user_id=" . $users[$k]['user_id'])->save($ke);
            }
        }
    }
}