...
class ReportController extends BaseController{
	...
	public function total(){
        ...
        $list = $Model->query("
                        select FROM_UNIXTIME(a.add_time,'%Y-%m-%d') as date, sum(a.total_amount) as total,sum(b.operations_fee) as operations , sum(b.surplus_amount) as surplus from tp_order  as a
             left join tp_order_amount_log  as b  ON  a.order_id = b.order_id 
            where pay_status=1 GROUP by FROM_UNIXTIME(a.add_time,'%Y-%m-%d')
        ");

        $amount = $Model->query("
           select  FROM_UNIXTIME(add_time,'%Y-%m-%d') as date, sum(amount) as amount from tp_system_amount_log GROUP by FROM_UNIXTIME(add_time,'%Y-%m-%d')
        ");

        foreach ($list as $k => $v) {
            foreach ($amount as $j => $i) {
                if ($list[$k]['date'] == $amount[$j]['date']) {
                    $list[$k]['amount'] =  $amount[$j]['amount'];
                    $list[$k]['receive'] = $list[$k]['total'] - $list[$k]['operations'] - $list[$k]['surplus'] - $list[$k]['amount'];
                }
            }
        }
...
/*    function totalList()
    {
        ...
        $count = M('Order')->where('pay_status =1')->count();
        $paget = new \Think\Page($count, 20);
        $show = $paget->show();

        $order = M("Order")
            ->alias('a')
            ->join('__USERS__ b ON a.user_id = b.user_id')
            ->where("pay_status=1")
            ->field(array('a.total_amount' => 'totalamount','add_time','first_leader','second_leader','third_leader'))
            ->limit($paget->firstRow.','.$paget->listRows)
            ->select();

        foreach ($order as $key => $value) {
            $order[$key]['date'] = date('Y-m-d',$order[$key]['add_time']+24*3600);
            $order[$key]['redeemable'] = $order[$key]['totalamount'] * 0.2;  // 返还金额
            if (!isset($order[$key]['first_leader'])) {
                $first_leader = $order[$key]['redeemable'] * 0.25;
            }
            if (!isset($order[$key]['second_leader'])) {
                $second_leader = $order[$key]['redeemable'] * 0.4;
            }
            if (!isset($order[$key]['third_leader'])) {
                $third_leader = $order[$key]['redeemable'] * 0.1;
            }
            $amout  = ($order[$key]['redeemable']*0.3) +($order[$key]['redeemable']*0.2)+($order[$key]['redeemable']*0.2)+($order[$key]['redeemable']*0.1)+($order[$key]['redeemable']*0.1);
            $leader = $first_leader + $second_leader + $third_leader;
            $order[$key]['redeemables'] = $amout + $leader;

            $order[$key]['business'] = $order[$key]['totalamount'] * 0.05;  // 运营成本
            $order[$key]['pool'] = $order[$key]['totalamount'] * 0.3;  // 资金池
            $order[$key]['other'] = $order[$key]['totalamount'] -$order[$key]['redeemables'] -$order[$key]['business']-$order[$key]['pool'];
        }
