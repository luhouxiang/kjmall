...
function update_user_level($user_id){
    ...
    $lower_count = M('users')->where("first_leader = $user_id ")->count();
    foreach($level_info as $k=>$v){
        if($v['amount']>$total_amount && $v['user_count'] > $lower_count && !isset($level)){
            if($k==0){
...
function update_pay_status($order_sn,$pay_status = 1)
{
    ...
    //设置权重
    set_weights($order['order_id']);
    
    // 购买商品 根据用户等级返积分
    setUserPoints($order);

    //设置商品支付后的资金池金额
    setSystemAmount($order);
     ...
     * @param array $order
     */
    function set_weights($userId){
    	//total_amount
    	$user = M('users')->where("user_id=$userId");
    	 
    	$child_user_lst = M('users')->where("first_leader=$userId")->select();
    	$weights = 0;
    	if(!isset($child_user_lst))
    	{
    		$user_id_arr = get_arr_column($child_user_lst, 'user_id');
    		$total_amount = D('order')->where("user_id in (".  implode(',', $user_id_arr).") AND order_status=2 or order_status=4")->sum('order_amount');
    		$weights = ($total_amount)/1000;
//     		$user->save(array("weights",$weights));
    	}
    	$total_amount = D('order')->where("user_id=$userId AND order_status=2 or order_status=4")->sum('order_amount');
    	$weights = ($total_amount)/1000 + $weights;
    	$user->save(array("weights",$weights));
    	//     	$second_leader_id = $users["second_leader"];
    }
     ...
     * @param array $order
     */
    function setUserPoints($order = array()){
    	$user_id = $order["user_id"];
    	 
    	$user = M('users')->where("user_id=$user_id")->find();
    	$level = $user['level'];
    	$user_level = M('user_level')->where("level_id = $level")->find();
    	$cash_back = $user_level['cash_back'];
    	 
    	$order_amount = $order["order_amount"] + $order["user_money"];
    	 
    	$pay_points = $cash_back * $order_amount ;
    	accountLog($user_id, 0 ,$pay_points,"购买商品 返积分: ".$pay_points );

    	//加入排队
    	addQueueByOrder($order,$pay_points);
    }
     ...
     * @param array $order
     */
    function addQueueByOrder($order = array(),$order_amount){
    	...
    	if($order_amount>100){
	    	...
	    	$queue = array(
	    			"user_id" => $order["user_id"],
	    			"join_time" => time(),
	    			"receive_amount" => $order_amount,
	    			"sequence" => $queueCount+1
	    	);
	    	M('user_queue')->add($queue);
    	}
     ...
     * @param array $orderId
     */
    function setSystemAmount($order = array()){
    	$order_amount = $order["order_amount"] + $order["user_money"];
    	$surplus_amount = $order_amount;
    	
    	//运营费用比例
    	$operations_fee_per = tpCache("shopping.operations_fee");
    	$operations_fee = $order_amount * $operations_fee_per;
    	
    	//分红的资金池比例
    	$amount_per = tpCache("shopping.system_amount");
    	$amount = $order_amount * $amount_per;
    	//分红的资金池
    	if($amount === 0 ){
    		return 0;
    	}
    	$system_amount = M('system_amount')->find();
    	if(isset($system_amount)){
    		$updateSql = "UPDATE __PREFIX__system_amount SET amount = amount + '$amount' where amount_id = ".$system_amount["amount_id"];
    		D('system_amount')->execute($updateSql);
    	}else{
    		$system_amount = array(
    				"amount"=>$amount
    		);
    		M('system_amount')->add($system_amount);
    	}
		...
		M('system_amount_log')->add($amount_log);
		$surplus_amount = $surplus_amount - $amount;
		
		//放入排队的资金池
		$amount_per = tpCache("shopping.system_amount_pd");
		$amount = $order_amount * $amount_per;
		//先计算该用户的 上级 、 上上级的反现值
		$user_id = $order["user_id"];
		$user = M('users')->where("user_id=$user_id")->find();
		...
		$surplus = 0;//自己没有返完的金额
		$user_back_per = tpCache("shopping.user_back");
		$user_amount = $amount * $user_back_per; //自己该返的金额
		$user_amount = $amount < $user_amount ? $amount : $user_amount;
		$surplus = backMoney($user,$user_amount);
		
		$p_user_amount = 0; //爸爸该返的金额
		$p_surplus = 0;//爸爸没有返完的金额
		$p_user = M('users')->where("user_id=".$user['first_leader'])->find();
		//上级的返现计算
		if(count($p_user)>0){
			$p_user_back = tpCache("shopping.p_user_back");
			$p_user_amount = $amount * $p_user_back;
// 			$p_user_amount = $amount < $p_user_amount ? $amount : $p_user_amount;
			/* if($p_user_amount < $amount - $user_amount){
				$p_user_amount = $p_user_amount ;
			}else{
				$p_user_amount = $amount - $user_amount;
			} */
			$p_surplus = backMoney($p_user,$p_user_amount);
		}
		
		$gp_surplus = 0;//爷爷该返的金额
		$gp_user_amount = 0;//爷爷没有返完的金额
		$gp_user = M('users')->where("user_id=".$user['second_leader'])->find();
		//上上级的返现
		if(count($p_user)>0){
			$gp_user_back = tpCache("shopping.gp_user_back");
			$gp_user_amount = $amount * $gp_user_back;
			/* if($gp_user_amount < $amount - $p_user_amount - $user_amount){
				$gp_user_amount = $gp_user_amount;
			}else{
				$gp_user_amount = $amount - $p_user_amount - $user_amount;
			} */
			$gp_surplus = backMoney($gp_user,$gp_user_amount);
		}
		
		$amount = $amount - ( $p_user_amount + $gp_user_amount + $user_amount ) + $p_surplus + $gp_surplus + $surplus ; 
		//分配的积分少于10不分了
		if($amount>10){
			$amount = back_queue($amount);
		}
		$system_amount_pd = M('system_amount_pd')->find();
		if(isset($system_amount_pd)){
			$updateSql = "UPDATE __PREFIX__system_amount_pd SET amount = amount + $amount  where amount_id = ".$system_amount["amount_id"];
			D('system_amount_pd')->execute($updateSql);
		}else{
			$system_amount_pd = array(
					"amount"=>$amount
			);
			M('system_amount_pd')->add($system_amount);
		}
		$amount_log = array(
				"amount"=>$amount,
				"user_id"=>$order["user_id"],
		...
		M('system_amount_pdlog')->add($amount_log);
		$surplus_amount = $surplus_amount - $amount - $operations_fee;
		//计算订单交易额的剩余金额
		$order_amount_log = array(
				"user_id"=>$order["user_id"],
				"order_id"=>$order["order_id"],
		...
		M('order_amount_log')->add($order_amount_log);
		
    }
    
    function back_queue($amount = 0){
    	if($amount === 0){
    		return 0;
    	}
    	$cash_back_lst = M()->query(" select * from __PREFIX__cash_back_queue order by cash_back_queue_id ");
    	//排队积分转换余额的规则，规则数量为正在排队领取的人数
    	$cash_back_per_arr = get_arr_column($cash_back_lst, 'cash_back_per');
    	if(array_sum($cash_back_per_arr)>1){
    		return 1;
    	}
    	$result = 0;

    	$user_queue_lst = M()->query(" select * from __PREFIX__user_queue  where is_receive = 1 order by sequence DESC , join_time desc limit ".count($cash_back_lst) );

        $user_queue_reapet = array();
        foreach ($user_queue_lst as $k => $v) {
            array_push($user_queue_reapet,$user_queue_lst[$k]['user_id']);
        }

        $count_values = array_count_values($user_queue_reapet);
        if (count(array_unique($user_queue_reapet)) < count($user_queue_reapet)) {
            $user_querue_reqpet1 = array();
            foreach ($count_values as $k => $v){
                if($v > 1){
                    array_push($user_querue_reqpet1,$k);
                }
            }
            foreach ($user_queue_lst as $k => $v) {
                foreach ($user_querue_reqpet1 as  $s) {

                    if ($user_queue_lst[$k]['user_id'] == $s) {
                        $queue = M("User_queue");

                        $data['sequence'] = $user_queue_lst[$k]['sequence'] + 4.9;
                        $data['is_receive'] = 0;
                        $queue->where("queue_id =" . $user_queue_lst[$k]['queue_id'])->save($data);
                        break 2;
                    }
                }
            }
            $user_queue_lst = M()->query(" select * from __PREFIX__user_queue as a left join __PREFIX__users as b  ON  a.user_id = b.user_id where is_receive = 1 order by b.weights asc, join_time desc limit ".count($cash_back_lst) );
    	...
    	if(count($user_queue_lst)<count($cash_back_lst)){
    		$count = count($cash_back_lst) - count($user_queue_lst);
    		$user_queue_add = M()->query(" select * from __PREFIX__user_queue  where is_receive = 0 order by sequence DESC , join_time desc limit ".$count );

            $user_queue_areapet = array();
            foreach ($user_queue_add as $k => $v) {
                array_push($user_queue_areapet,$user_queue_add[$k]['user_id']);
            }
            $count_avalues = array_count_values($user_queue_areapet);
            if (count(array_unique($user_queue_areapet)) < count($user_queue_areapet)) {
                $user_querue_areqpet1 = array();
                foreach ($count_avalues as $k => $v) {
                    if ($v > 1) {
                        array_push($user_querue_areqpet1, $k);
                    }
                }
                foreach ($user_queue_add as $k => $v) {
                    foreach ($user_querue_areqpet1 as $s) {

                        if ($user_queue_add[$k]['user_id'] == $s) {
                            $queue = M("User_queue");

                            $data['sequence'] = $user_queue_add[$k]['sequence'] + 4.9;
                            $data['is_receive'] = 0;
                            $queue->where("queue_id =" . $user_queue_add[$k]['queue_id'])->save($data);
                            break 2;
                        }
                    }
                }
                $user_queue_add = M()->query(" select * from __PREFIX__user_queue as a left join __PREFIX__users as b a.user_id = b.user_id  where is_receive = 0 order by b.weights asc, join_time desc limit ".$count );
    		...
    		if(count($user_queue_lst) == 0){
    			$user_queue_lst = $user_queue_add;
    		}elseif(count($user_queue_lst)<count($cash_back_lst)){
    			// 在领取积分的排队里面，每个人只能有个账号 ，如果整个排队小于需要排队的人例外
    			foreach ($user_queue_add as $queue_add){
    				$user_queue_lst[] = $queue_add;
    			}
    			
//     			array_push($user_queue_lst, $user_queue_add);
    			
    		}
    	}
    	while(current($user_queue_lst)){
    		$user_queue = current($user_queue_lst);
    		$user_id = $user_queue["user_id"];
    		$cash_back_per = current($cash_back_per_arr);
    		
    		$user = M('users')->where("user_id=$user_id")->find();
    		$points = $cash_back_per * $amount;
    		if($points!=0)
    			$result = $result + $points - backMoney($user,$points);
    		next($user_queue_lst);
    		next($cash_back_per_arr);
    	}
    	return $result;
    }
     ...
     * @param number $points 需要兑换的积分值
     */
    function backMoney($user = array(),$points = 0){
    	if(count($user) == 0)
    		return 0;
    	...
    	$result = $points;
    	$pay_point = 0;
//     	$user_queue_lst = M()->query(" select * from __PREFIX__user_queue where is_receive in ( 0 , 1 ) and user_id = $user_id order by sequence asc, join_time desc ")->find();
    	$user_queue_lst = M("user_queue")->where("is_receive in ( 0 , 1 ) and user_id = $user_id ")->order("sequence asc, join_time desc ")->find();
   		$user_point = $user["pay_points"];
    	if(count($user_queue_lst) > 0 and !empty($user_point) ){
    		$queue_point = $user_queue_lst["receive_amount"];

//     		$pay_point = $queue_point < $points ? $queue_point : $points ;
//     		$pay_point = $user_point < $pay_point ? $user_point : $pay_point ;
			$param=array();
			if($user_point < ($queue_point)){
				$pay_point = $user_point;
				$param=array(
						"receive_amount" => 0,
						"received_amount" => $user_point == 0 ? 0 : $user_queue_lst["received_amount"] + $user_point,
						"is_receive" => 2
				);
				$result = $points - $user_point;
			}else{
				//判断该排队号需要领取多少钱
				$param_log = array();
				if($queue_point <= $points){
					$pay_point = $queue_point;
					$result = $points - $queue_point;
					$param = array(
							"receive_amount" => 0,
							"received_amount" => $user_queue_lst["received_amount"] + $pay_point,
							"is_receive" => 2
					);
				}else {
					$result = 0;
					$pay_point = $points;
					$param = array(
							"receive_amount" => $queue_point - $pay_point,
							"received_amount" => $user_queue_lst["received_amount"] + $pay_point,
							"is_receive" => 1
					);
				}
    		...
    		M("user_queue")->where("queue_id=".$user_queue_lst["queue_id"])->save($param);
    		
    		$param_log = array(
    				"queue_id" => $user_queue_lst["queue_id"],
    				"receive_time" => time(),
    				"receive_amount" => $pay_point,
    		);
    		M("user_queue_log")->add($param_log);

			
    		if($user_point < $pay_point){
    			$pay_point = $user_point;
    		}
    		
    		accountLog($user["user_id"], $pay_point ,0-$pay_point,"用户积分返现: ".$pay_point );
