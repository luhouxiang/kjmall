    ...
    */
    ...
    public function queue(){
    	...
    	$usSql = "select uq.*,us.nickname from __PREFIX__user_queue uq inner join __PREFIX__users us on uq.user_id = us.user_id
    			where is_receive in ( 0 , 1 ) order by is_receive desc,sequence asc, join_time desc";
    	$queue_lst = M()->query($usSql);
    	$usSql = "select uq.*,us.nickname from __PREFIX__user_queue uq inner join __PREFIX__users us on uq.user_id = us.user_id
    	where is_receive in ( 0 , 1 ) and uq.user_id = $this->user_id order by is_receive desc,sequence asc, join_time desc";
    ...
    public function integral_record(){
    	...
    	$queue_lst = M()->query($usSql);
    	$count = count($queue_lst);
    	$page = new Page($count,10);
    	$p = $page->show();
    	$usSql = $usSql." limit {$page->firstRow},{$page->listRows} ";
    	$queue_lst = M()->query($usSql);
    	$this->assign('queue_lst',$queue_lst);
    	$this->assign('page',$page->show());
    	$this->display();
