...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        {
            $flash_sale = M('flash_sale')->where("id = {$goods['prom_id']}")->find(); // 限时抢购活动
            ...
            {  
                ...
                return array('status'=>-4,'msg'=>"每个用户限购 {$flash_sale['buy_limit']}件 $error_msg",'result'=>'');
            }              
        }                
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
    	...
    	if(($car_price['goodsFee'] + $car_price['postFee'])>2000)
    		return array('status'=>-9,'msg'=>'每单限额2000元','result'=>'');
    	
        // 仿制灌水 1天只能下 50 单  // select * from `tp_order` where user_id = 1  and order_sn like '20151217%' 
        $order_sum = M('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->sum("total_amount"); // 查找一天内订单的总值
        if($order_sum>2000)
        	return array('status'=>-9,'msg'=>'每日限额2000元','result'=>'');
        
        
        $order_count = M('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
        if($order_count >= 20) 
            return array('status'=>-9,'msg'=>'一天只能下20个订单','result'=>'');    
        
        $randStr = str_shuffle('1234567890');
        $rand = substr($randStr,0,4);
//         $order_count = M('Order')->where("rand = '".$rand."'")->count();
//         while(!empty($order_count)){
